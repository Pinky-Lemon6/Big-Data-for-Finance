...
def get_params():
    ...
    if not os.path.exists(os.path.join(current_directory, "./mean_var.txt")):
        ...
        data_dir = os.path.join(
            current_directory, "../Dataset/S&P500/all_stocks_5yr.csv"
        )
...
def main():
    ...
    val_loader = DataLoader(
        val_db, batch_size=batch_size, shuffle=False, num_workers=cores
    )
    test_loader = DataLoader(
        test_db, batch_size=batch_size, shuffle=False, num_workers=cores
    )
            ...
            if batch_idx % 100 == 0:
                print(
                    "Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}".format(
                        ...
                        batch_idx * len(X),
                        train_db.__len__(),
                        100.0 * batch_idx * batch_size / train_db.__len__(),
                        loss.item(),
                    )
                )
