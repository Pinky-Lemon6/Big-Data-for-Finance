...
class StockDataset(Dataset):
    ...
    def __init__(self, dim_x=9, mode="train"):
        ...
        # self.split_date_tv = "2016-04-14" # 划分train、val
        self.split_date_tv = "2016-04-01" # 划分train、val
        self.split_date_vt = "2017-01-30" # 划分val、test所有日期>=split_date的数据均为测试集
    ...
    def load_dataset(self):
        if self.mode == "train":
            dataset_name = f"all_stocks_5yr_clean_{self.dim_x}T_train_tvt.csv"
            if not os.path.exists(os.path.join(self.root, dataset_name)):
                ...
                for idx, file_name in enumerate(self.file_list):
                    ...
                    data = data[data.date<self.split_date_tv]
                    data = data.sort_values(by = "date", ascending=True).reset_index()
                    for i in range(0, len(data)-self.dim_x-1, self.dim_x+1):
            ...
            dataset_name = f"all_stocks_5yr_clean_{self.dim_x}T_val_tvt.csv"
            if not os.path.exists(os.path.join(self.root, dataset_name)):
                print(f"{dataset_name} does not exist! creating...")
                self.file_list = self.load_csv()
                for idx, file_name in enumerate(self.file_list):
                    data = pd.read_csv(file_name)
                    data["date"] = data["date"].apply(self.standard_date)
                    data = data[(data.date>=self.split_date_tv)&(data.date<self.split_date_vt)]
                    data = data.sort_values(by = "date", ascending=True).reset_index()
                    for i in range(self.dim_x, len(data)-self.dim_x-1, self.dim_x+1):
                        x = data.loc[i:i+self.dim_x, "close"].to_list() # X=x[:-1], y=x[-1]
                        with open(os.path.join(self.root, dataset_name), "a") as f:
                            f.write(",".join([str(i) for i in x]))
                            f.write("\n")
            ...
            dataset_name = f"all_stocks_5yr_clean_{self.dim_x}T_test_tvt.csv"
            if not os.path.exists(os.path.join(self.root, dataset_name)):
                print(f"{dataset_name} does not exist! creating...")
                self.file_list = self.load_csv()
                for idx, file_name in enumerate(self.file_list):
                    data = pd.read_csv(file_name)
                    data["date"] = data["date"].apply(self.standard_date)
                    data = data[data.date>=self.split_date_vt]
                    data = data.sort_values(by = "date", ascending=True).reset_index()
                    for i in range(self.dim_x, len(data)-self.dim_x-1, self.dim_x+1):
                        x = data.loc[i:i+self.dim_x, "close"].to_list() # X=x[:-1], y=x[-1]
                        with open(os.path.join(self.root, dataset_name), "a") as f:
...
if __name__ == "__main__":
    ...
    val_db = StockDataset(mode="val", dim_x=9)
    ...
    print("train: ", train_db.__len__())
    for X, y in train_loader:
        print(X.size(), y)
        break
    print("test: ", test_db.__len__())
    for X, y in test_loader:
        print(X.size(), y)
        break
    print("val: ", val_db.__len__())
